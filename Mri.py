
import torch
import torch.nn as nn
import torch.optim as optim
import numpy as np
import matplotlib.pyplot as plt
import requests
from PIL import Image
from io import BytesIO
import os
import easygui


# Define a more complex CNN architecture
class TumorSegmentationModel(nn.Module):
    def __init__(self):
        super(TumorSegmentationModel, self).__init__()
        self.conv1 = nn.Conv2d(1, 16, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(16)
        self.conv2 = nn.Conv2d(16, 32, kernel_size=3, padding=1)
        self.bn2 = nn.BatchNorm2d(32)
        self.conv3 = nn.Conv2d(32, 64, kernel_size=3, padding=1)
        self.bn3 = nn.BatchNorm2d(64)
        self.conv4 = nn.Conv2d(64, 32, kernel_size=3, padding=1)
        self.bn4 = nn.BatchNorm2d(32)
        self.conv5 = nn.Conv2d(32, 16, kernel_size=3, padding=1)
        self.bn5 = nn.BatchNorm2d(16)
        self.conv6 = nn.Conv2d(16, 1, kernel_size=3, padding=1)
        self.relu = nn.ReLU()
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        x = self.relu(self.bn1(self.conv1(x)))
        x = self.relu(self.bn2(self.conv2(x)))
        x = self.relu(self.bn3(self.conv3(x)))
        x = self.relu(self.bn4(self.conv4(x)))
        x = self.relu(self.bn5(self.conv5(x)))
        x = self.sigmoid(self.conv6(x))
        return x

def load_mri_image(image_path):
    # Load and preprocess the MRI image (e.g., normalize, resize)
    # Return the preprocessed image
    if image_path.startswith('http'):
        response = requests.get(image_path)
        image = Image.open(BytesIO(response.content))
    else:
        image = Image.open(image_path)
    image_np = np.array(image)
    # Convert to grayscale if necessary
    if image_np.ndim == 3:
        image_np = np.mean(image_np, axis=2)
    # Add any necessary preprocessing steps here (e.g., normalization, resizing)
    return image_np  # Return the NumPy array here


def main():
    # Ask the user to select the image using a dialog box
    image_path = easygui.fileopenbox(title="Select MRI Image", default="*", filetypes=["*.jpg", "*.png", "*.jpeg"])

    if image_path is None:
        print("No image selected. Exiting program.")
        return

    # Load and preprocess the MRI image
    mri_image = load_mri_image(image_path)

    # Prepare input tensor (assuming grayscale image)
    input_tensor = torch.from_numpy(mri_image).unsqueeze(0).unsqueeze(0).float()

    # Load the model
    model = TumorSegmentationModel()
    # Update the path to your actual model file


    # Generate segmentation mask
    with torch.no_grad():
        predicted_mask = model(input_tensor)

    # Visualize the original image and the predicted mask
    plt.figure(figsize=(15, 10))
    plt.subplot(1, 2, 1)
    plt.imshow(mri_image, cmap="gray")
    plt.title("Original MRI")
    plt.axis("off")

    plt.subplot(1, 2, 2)
    plt.imshow(predicted_mask[0, 0].detach().numpy(), cmap="coolwarm", vmin=0, vmax=1) # Convert tensor to NumPy and detach from graph
    plt.title("Predicted Tumor Mask")
    plt.axis("off")
    plt.legend(['No Tumor', 'Tumor'], fontsize='x-large')

    plt.show()

    # Add segmentation result to the original image
    segmented_image = np.copy(mri_image)
    segmented_image[predicted_mask[0, 0].detach().numpy() > 0.5] = 255

    # Visualize the original image, predicted mask, and segmented image
    plt.figure(figsize=(15, 10))
    plt.subplot(1, 3, 1)
    plt.imshow(mri_image, cmap="gray")
    plt.title("Original MRI")
    plt.axis("off")

    plt.subplot(1, 3, 2)
    plt.imshow(predicted_mask[0, 0].detach().numpy(), cmap="coolwarm", vmin=0, vmax=1)
    plt.title("Predicted Tumor Mask")
    plt.axis("off")
    plt.legend(['No Tumor', 'Tumor'], fontsize='x-large')

    plt.subplot(1, 3, 3)
    plt.imshow(segmented_image, cmap="gray")
    plt.title("Segmented MRI")
    plt.axis("off")
    plt.legend(['No Tumor', 'Tumor'], fontsize='x-large')

    plt.show()

    # Print prediction
    if predicted_mask[0, 0].detach().numpy().mean() > 0.5:
        print("The MRI scan shows a tumor.")
    else:
        print("The MRI scan does not show a tumor.")

    # Check the average value of the predicted mask
    average_mask_value = predicted_mask[0, 0].detach().numpy().mean()

    # If the average mask value is close to 0.5, it means the model is uncertain about the presence of a tumor
    if 0.45 < average_mask_value < 0.55:
        print("The model is uncertain about the presence of a tumor in this MRI scan.")

if __name__ == "__main__":
    main()

